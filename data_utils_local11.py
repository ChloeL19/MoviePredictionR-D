...
def get_feature_directory(feature_name, movie_num):
    if movie_num < 10:
        path = os.path.join("/Users/chloeloughridge/Desktop/AI Grant Project/Dataset/2018 Task Data/MEDIAEVAL18-DevSet-Part1-Visual_features/visual_features/MEDIAEVAL18_0{}".format(movie_num), 
                            "{}".format(feature_name))
    else:
        path = os.path.join("/Users/chloeloughridge/Desktop/AI Grant Project/Dataset/2018 Task Data/MEDIAEVAL18-DevSet-Part1-Visual_features/visual_features/MEDIAEVAL18_{}".format(movie_num), 
                            "{}".format(feature_name))
    return path

# a function for iterating through all of the files in a folder and loading them into input_data
def load_Xinput(directory, timesteps, data_dim):
    X_input = np.zeros([timesteps, data_dim]) 
    count = 0
...
# get audio feature directory
def get_audio_dir(movie_num):
    if movie_num < 10:
        path = os.path.join("/Users/chloeloughridge/Desktop/AI Grant Project/Dataset/2018 Task Data/MEDIAEVAL18-DevSet-Part1-Audio_features/audio_features/MEDIAEVAL18_0{}".format(movie_num))
    else:
        path = os.path.join("/Users/chloeloughridge/Desktop/AI Grant Project/Dataset/2018 Task Data/MEDIAEVAL18-DevSet-Part1-Audio_features/audio_features/MEDIAEVAL18_{}".format(movie_num))
    return path


# loading frames for creating new features
def load_Xinput(directory, timesteps, data_dim):
    X_input = np.zeros([timesteps, data_dim]) 
    count = 0
    for file in os.listdir(directory):
